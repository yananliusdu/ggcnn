...
class BoschDataset(GraspDatasetBase):
    """
    ...
    def __init__(self, file_path, start=0.0, end=1.0, ds_rotate=0, **kwargs):
        ...
        """
        super(BoschDataset, self).__init__(**kwargs)

        graspf = glob.glob(os.path.join(file_path, '*', '*pos.txt'))
        # graspf.sort()
        ...
        depthf = [f.replace('_annotations_pos.txt', '_depth.tiff') for f in graspf]
        rgbf = [f.replace('_depth.tiff', '.png') for f in depthf]
